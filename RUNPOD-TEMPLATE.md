...
| `OPENCLAW_DISABLE_BONJOUR` | Нет | По умолчанию **`1`** в образе: отключает mDNS/Bonjour в `openclaw.json`, чтобы на RunPod не было crash loop шлюза. LAN discovery: **`0`**. |
...
`WEB_SEARCH_PROVIDER`, `HF_TOKEN`-связанные URL дефолтной модели, `BOOTSTRAP_GGUF`, `OPENCLAW_LOCAL_MODEL_INPUT`, `OPENCLAW_DISABLE_BONJOUR`, `LLAMA_SERVER_EXTRA_ARGS`, `OPENCLAW_STATE_DIR`, `OPENCLAW_CONTROL_UI_*`, `TELEGRAM_FORCE` и др. Старые имена с префиксом **`GUILDCLAW_*`** в образе по-прежнему подхватываются как fallback.
