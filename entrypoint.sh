...
# Bonjour/mDNS в контейнере и на RunPod часто ломает шлюз (unhandled CIAO…); см. DEPLOY.md.
...
export MODEL_HUB_TOKEN PAIRING_DASH_TOKEN ENABLE_JUPYTER JUPYTER_LAB_TOKEN BOOTSTRAP_GGUF \
    ...
    OPENCLAW_LOCAL_MODEL_INPUT LOCAL_LLAMA_VISION OPENCLAW_DISABLE_BONJOUR
...
export OPENCLAW_WEB_PASSWORD HF_HOME OPENCLAW_STATE_DIR LLAMA_API_KEY SERVED_MODEL_NAME LLAMA_CTX_SIZE LLAMA_N_GPU_LAYERS OPENCLAW_COMPACTION_RESERVE_TOKENS_FLOOR OPENCLAW_COMPACTION_PROMPT_HEADROOM OPENCLAW_LOCAL_MODEL_INPUT LOCAL_LLAMA_VISION OPENCLAW_DISABLE_BONJOUR
