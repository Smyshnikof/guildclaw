...
| `GUILDCLAW_WEB_SEARCH` | `1` / `true` / `yes` / `on` — при каждом старте пода в `openclaw.json` выставляется **`tools.web.search.provider`: `duckduckgo`** ([дока OpenClaw](https://docs.openclaw.ai/tools/duckduckgo-search)). Пусто — не трогаем секцию `tools` (можно задать провайдер вручную). |
| `GUILDCLAW_WEB_SEARCH_PROVIDER` | Явное имя провайдера (`duckduckgo`, `searxng`, …), если нужно не короткое `GUILDCLAW_WEB_SEARCH=1`. Если задано непустое значение, оно **перекрывает** автоматический выбор из `GUILDCLAW_WEB_SEARCH`. |
