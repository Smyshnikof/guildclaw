...
Файлы GGUF: `/workspace/models/gguf/`. Состояние: **`/workspace/.guildclaw/active.json`** (`path`, `served_id`, опционально `llama_ctx_size`, опционально **`openclaw_model_input`**, опционально **`llama_mmproj`**). Для **Qwen3.5 VL** в пресете **a2go** в одном профиле с HF тянут **два** файла: основной вес и **`mmproj-*.gguf`** — `llama-server` получает [`--mmproj`](https://github.com/ggml-org/llama.cpp/blob/master/tools/server/README.md). Раньше Guildclaw при автозагрузке качал только первый файл при **`LOCAL_LLAMA_VISION=1`**, из‑за этого шлюз слал картинки в сервер **без** проекции → ошибки API и **failover**. Сейчас при дефолтном бутстрапе второй файл тоже качается и пишется в **`active.json`**; если mmproj уже лежит **в том же каталоге**, что активный `.gguf`, entrypoint подставит **`--mmproj`** сам. **`LOCAL_LLAMA_VISION`** / Hub «text+image» — про отправку изображений **из OpenClaw**; без mmproj на стороне llama это всё равно не оживит **VL**-ветку.
...
| `LOCAL_LLAMA_VISION` | `1` / `true` — в `openclaw.json` у **local-llama** в карточке модели **`"input": ["text", "image"]`** (шлюз шлёт картинки в API). **`0`** — только текст. Для VL на стороне **llama-server** нужен **`--mmproj`** (см. **`DEFAULT_GGUF_MMPROJ_*`**, **`llama_mmproj`**, авто-поиск `mmproj*.gguf` рядом с моделью, **`LLAMA_SERVER_EXTRA_ARGS`**). |
...
| `OPENCLAW_DISABLE_BONJOUR` | По умолчанию **`1`**: при каждом запуске **`sync_openclaw_llama.py`** дописывает в **`openclaw.json`** отключение плагина **`plugins.entries.bonjour`** и **`discovery.mdns.mode`: `"off"`** (на RunPod/VPS без mDNS плагин иначе даёт **unhandled rejection** `CIAO ANNOUNCEMENT CANCELLED` и цикл перезапусков шлюза). **`0`** / **`false`** — не трогать конфиг (нужен LAN Bonjour). Переменная **`OPENCLAW_DISABLE_PLUGINS`** в апстриме **не** является официальным переключателем; корректная схема — **`plugins.entries`** и **`discovery.mdns`**, см. [configuration reference](https://docs.openclaw.ai/gateway/configuration-reference). |
...
| `DEFAULT_GGUF_MMPROJ_URL` | URL второго файла проекции (по умолчанию **`mmproj-F16.gguf`** из того же репозитория, что и дефолтная Qwen3.5). Пустое значение — не качать второй файл при бутстрапе. |
| `DEFAULT_GGUF_MMPROJ_FILENAME` | Имя файла mmproj на диске (по умолчанию **`mmproj-F16.gguf`**) |
| `BOOTSTRAP_GGUF_MMPROJ` | `1` (по умолчанию): при бутстрапе скачать mmproj, если задан **`DEFAULT_GGUF_MMPROJ_URL`**. `0` — только основной GGUF (например своя текстовая модель по **`DEFAULT_GGUF_URL`**) |
...
**Устаревшие имена** (в entrypoint и Python по-прежнему читаются как fallback): `GUILDCLAW_HUB_TOKEN`, `GUILDCLAW_PAIRING_DASH_TOKEN`, `GUILDCLAW_JUPYTER`, `GUILDCLAW_JUPYTER_TOKEN`, `GUILDCLAW_WEB_SEARCH`, `GUILDCLAW_WEB_SEARCH_PROVIDER`, `GUILDCLAW_BOOTSTRAP_GGUF`, `GUILDCLAW_BOOTSTRAP_GGUF_MMPROJ`, `GUILDCLAW_DEFAULT_GGUF_URL`, `GUILDCLAW_DEFAULT_GGUF_FILENAME`, `GUILDCLAW_DEFAULT_GGUF_MMPROJ_URL`, `GUILDCLAW_DEFAULT_GGUF_MMPROJ_FILENAME`, `GUILDCLAW_TELEGRAM_FORCE`, `GUILDCLAW_STATE_DIR`, `GUILDCLAW_GGUF_DIR`, `GUILDCLAW_LLAMA_PID_FILE`, `GUILDCLAW_SYNC_SCRIPT`, `GUILDCLAW_PAIRING_DEVICE_JSON`, `GUILDCLAW_MATE_CWD`, `GUILDCLAW_LOCAL_LLAMA_VISION`, `GUILDCLAW_OPENCLAW_LOCAL_MODEL_INPUT`.
